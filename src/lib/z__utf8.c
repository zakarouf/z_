...
inline static z__u8 utf8_eval_codepoint(z__byte const c)
{
    z__u8 num_bytes = 1;
...
inline static z__u8 utf8_char_is_valid(z__byte const c)
{
					   ...
					   , z__size size)
{
    ...
    while(utf8_char_is_valid(*str)) { str+=1; }
    return str == str_end;
}
...
{
    z__u8 const *str_end = str + size;
    while (utf8_char_is_valid(*str) == 1) { str+=1; }
    return str == str_end;
}
					  ...
					  , z__size size)
{
    z__u8 const *str_end = str + size;
    z__size len = 0;
    while(str < str_end) { str += utf8_eval_codepoint(*str); 
                           len += 1; }
    return len;
}
